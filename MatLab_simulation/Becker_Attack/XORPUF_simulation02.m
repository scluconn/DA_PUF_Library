...
%We want to see that with the same set of CRPs, applying the CMAES many
%times then can we see different model?
...
%We want to see the models can be built 

SeenAPUFs  = zeros(nXOR,1);

for k=1:U
    for j=1:nXOR 
        if(matchingRateMultipleTimes(k,j)==1)
