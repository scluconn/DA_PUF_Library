function [lambda,wModel] = CMAES_XorPUF(x,chalSize,nTrain, ...
                               trainSetPhi,trainSetResponses)
...
% --------------------  Initialization --------------------------------  
  ...
  strfitnesfct = 'modelAcc_rel_du_XorPUF';  % name of objective/fitness function
  Size = x*(chalSize+1);
  N = Size;               % number of objective variables/problem dimension
  xmean = rand(N,1);    % objective variables initial point
  sigma = 0.5;          % coordinate wise standard deviation (step size)
  stopfitness = 1e-10;  % stop if fitness < stopfitness (minimization)
  stopeval = 5000; %1e3*N^2;   % stop after stopeval number of function evaluations
  ...
  while counteval < stopeval
    ...
    for k=1:lambda,
      ...
      arfitness(k) = feval(strfitnesfct, arx(:,k),x,chalSize,  ... 
                            trainSetPhi,trainSetResponses,nTrain); % objective function call
