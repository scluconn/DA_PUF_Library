...
{
    [Serializable]
    class ArbiterPUF : PhysicallyUncloneableFunction, ICloneable
    {
        ...
        //Takes a phi vector and computes the response of the arbiter PUF (noisy)
        public sbyte ComputeNoisyResponseStandard(sbyte[] phiChallenge)
        {
            //error checking
            if (phiChallenge.Length != (bitNumber + 1))
            {
        ...
        public sbyte ComputeNoisyResponseMajorityVoting(sbyte[] phiChallenge)
        {
            sbyte finalResponse;
            int zerosCount = 0;
            int onesCount = 0;
